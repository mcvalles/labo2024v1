...
envg$EXPENV <- list()
envg$EXPENV$dir <- "~/buckets/b1/exp2-pca/"
envg$EXPENV$exp_dir <- "~/buckets/b1/exp2-pca/exp/"
envg$EXPENV$wf_dir <- "~/buckets/b1/exp2-pca/flow/"
...
corrida_guantesblancos_202109 <- function( pnombrewf, pvirgen=FALSE )
{
  ...
  dataset_dir <- paste0( "", envg$EXPENV$exp_dir, "DR0001/dataset.csv.gz" )
  
  # Verificar si la reducción de dimensionalidad es exitosa
  if (!Reduccion_Dimensionalidad(dataset_dir)) {
    stop("Error: La reducción de dimensionalidad no fue exitosa.")
  }
  
  TS_strategy_guantesblancos_202109( "TS0001", "DR0001" )
...
corrida_guantesblancos_202107 <- function( pnombrewf, pvirgen=FALSE )
{
  ...
  TS_strategy_guantesblancos_202107( "TS0002", "DR0001" )
